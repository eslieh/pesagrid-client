...
export async function getTransactions(params = {}) {
  ...
  if (params.psp_config_id) query.append("psp_config_id", params.psp_config_id);
  if (params.status) query.append("status", params.status);
  if (params.collection_point_id) query.append("collection_point_id", params.collection_point_id);
  if (params.unmatched_only) query.append("unmatched_only", "true");
...
/**
 * Manually match or re-assign a transaction
 * @param {string} transactionId 
 * @param {Object} data - { obligation_id, collection_point_id }
 * @returns {Promise<Object>}
 */
export async function matchTransaction(transactionId, data) {
  const res = await authenticatedFetch(`/api/v1/transactions/${transactionId}/match`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
  return handleResponse(res, "Failed to match transaction");
}
