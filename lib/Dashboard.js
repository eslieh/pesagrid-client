 ...
 */
export async function getDashboardMetrics(collectionPointId = null) {
  const query = collectionPointId ? `?collection_point_id=${collectionPointId}` : "";
 ...
 */
export async function getRecentPayments(collectionPointId = null, skip = 0, limit = 50) {
  const params = new URLSearchParams();
  if (collectionPointId) params.append("collection_point_id", collectionPointId);
  params.append("skip", skip);
  params.append("limit", limit);
