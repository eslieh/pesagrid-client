...
import { getObligations } from "../../../../lib/Obligation";
...
export default function ManualMatchModal({ isOpen, onClose, transaction, formatCurrency, onMatched }) {
  ...
  const handleSearch = useCallback(async (query) => {
    ...
    try {
      if (targetType === "obligation") {
        // Fetch by explicit account number instead of general search, and remove status constraint
        const res = await getObligations({ account_no: query, limit: 10 });
        setResults(res.items || []);
        if (!res.items || res.items.length === 0) {
          setError(`No obligation found with account number "${query}"`);
        }
      } else {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="px-8 py-6 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
            <div>
              <h2 className="text-[17px] font-black text-zinc-900 leading-tight">Match Transaction</h2>
          ...
          <div className="p-8">
            ...
            {step === 1 && (
              <div className="space-y-6">
                <h3 className="text-[15px] font-bold text-zinc-800">Assign this transaction to:</h3>
                <div className="grid grid-cols-2 gap-4">
                  <button 
                    onClick={() => { setTargetType("obligation"); nextStep(); }}
                  ...
                  >
                    <div className="h-12 w-12 rounded-2xl bg-zinc-50 flex items-center justify-center group-hover:bg-[#a3e635]/20 transition-colors">
                      <svg className="h-6 w-6 text-zinc-400 group-hover:text-[#65a30d]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-[14px] font-black text-zinc-900">Specific Invoice</p>
                      <p className="text-[11px] text-zinc-500 mt-1 font-medium">Match to a payer's obligation</p>
                    </div>
                  </button>

                  <button 
                    onClick={() => { setTargetType("collection_point"); nextStep(); }}
                  ...
                  >
                    <div className="h-12 w-12 rounded-2xl bg-zinc-50 flex items-center justify-center group-hover:bg-blue-100 transition-colors">
                      <svg className="h-6 w-6 text-zinc-400 group-hover:text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-[14px] font-black text-zinc-900">Collection Point</p>
                      <p className="text-[11px] text-zinc-500 mt-1 font-medium">Categorize under a point</p>
                    </div>
                  </button>
                </div>
              </div>
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div className="flex items-center gap-2">
                  <button onClick={prevStep} className="p-2 -ml-2 rounded-full hover:bg-zinc-100 text-zinc-400">
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
                    </svg>
                  </button>
                  <h3 className="text-[15px] font-bold text-zinc-800">Select target {targetType?.replace('_', ' ')}</h3>
                </div>

                <div className="flex flex-col gap-2">
                  <p className="text-[12px] font-medium text-zinc-500">
                    {targetType === 'obligation' 
                      ? "Please enter the exact Payer Account Number to find the obligation." 
                      : "Enter the Collection Point name or account number."}
                  </p>
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <svg className="absolute left-4 top-3.5 h-4 w-4 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                      </svg>
                      <input 
                        autoFocus
                        type="text"
                        placeholder={targetType === 'obligation' ? "e.g. ACC1002" : "Search..."}
                        value={search}
                ...
                <div className="max-h-[300px] overflow-y-auto pr-2 custom-scrollbar space-y-2">
                  {results.length > 0 ? (
                    results.map((item) => {
                      const payer = targetType === 'obligation' ? item.payer : null;
                      const displayName = targetType === 'obligation' ? (payer?.name || item.payer_name) : item.name;
                      ...
                      return (
                        <button
                          key={item.id}
                          onClick={() => { setSelectedTarget(item); nextStep(); }}
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 text-[11px] font-bold text-zinc-400 uppercase tracking-tight">
                              <span className="text-zinc-600">#{item.account_no}</span>
                              <span className="h-1 w-1 rounded-full bg-zinc-200" />
                              <span className="truncate">{item.description}</span>
                              {displayPhone && (
                                <>
                                  <span className="h-1 w-1 rounded-full bg-zinc-200" />
                                  <span className="tabular-nums text-zinc-500">{displayPhone}</span>
                                </>
                              )}
                            </div>
                            {displayEmail && (
                              <p className="text-[10px] font-medium text-zinc-400 mt-1">{displayEmail}</p>
                            )}
                          ...
                          <div className="text-right ml-4">
                             ...
                             <span className="text-[10px] font-black uppercase text-[#65a30d] opacity-0 group-hover:opacity-100 transition-all block mt-1">Select →</span>
                          </div>
                        </button>
                      );
                    })
            ...
            {step === 3 && selectedTarget && (
              <div className="space-y-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-[15px] font-bold text-zinc-800">Confirm Assignment</h3>
                </div>
                ...
                <div className="p-6 rounded-[32px] bg-zinc-50 border border-zinc-100 space-y-6 shadow-inner">
                  <div className="flex items-center justify-between pb-6 border-b border-zinc-200">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                         <p className="text-[15px] font-black text-zinc-900">{transaction.psp_ref}</p>
                      ...
                      <p className="text-[13px] text-zinc-500 font-bold mt-1">
                        Amount: <span className="text-zinc-900">{formatCurrency(transaction.amount)}</span>
                      </p>
                    </div>
                  ...
                  <div className="relative pl-6 border-l-2 border-[#a3e635]/30">
                    <SectionLabel>Target {targetType === 'obligation' ? 'Invoice' : 'Collection Point'}</SectionLabel>
                    <div className="mt-2">
                      <p className="text-[16px] font-black text-zinc-900 leading-tight">
                        {targetType === 'obligation' ? (selectedTarget.payer?.name || selectedTarget.payer_name) : selectedTarget.name}
                      </p>
                      
                      <div className="grid grid-cols-2 gap-y-3 mt-4 text-[12px]">
                        <div>
                          <p className="text-zinc-400 font-bold uppercase text-[9px] tracking-widest">Account #</p>
                          <p className="font-bold text-zinc-700">{selectedTarget.account_no}</p>
                        </div>
                        {targetType === 'obligation' && (
                        <div>
                          <p className="text-zinc-400 font-bold uppercase text-[9px] tracking-widest">Current Balance</p>
                          <p className="font-bold text-[#65a30d]">KES {selectedTarget.balance.toLocaleString()}</p>
                        </div>
                        )}
                        <div className="col-span-2">
                          <p className="text-zinc-400 font-bold uppercase text-[9px] tracking-widest">Invoice Description</p>
                          <p className="font-medium text-zinc-600 italic">"{selectedTarget.description}"</p>
                        </div>
                        {(selectedTarget.payer?.phone || selectedTarget.phone || selectedTarget.payer?.email) && (
                        <div className="col-span-2">
                          <p className="text-zinc-400 font-bold uppercase text-[9px] tracking-widest">Payer Contact</p>
                          <div className="space-y-0.5">
                            { (selectedTarget.payer?.phone || selectedTarget.phone) && <p className="font-bold text-zinc-700">{selectedTarget.payer?.phone || selectedTarget.phone}</p> }
                            { selectedTarget.payer?.email && <p className="font-medium text-zinc-500">{selectedTarget.payer.email}</p> }
                          </div>
                        </div>
                        )}
                      </div>
                ...
                <p className="text-[11px] text-center text-zinc-500 font-medium px-6 leading-relaxed">
                  Note: This will asynchronously trigger the reconciliation process. This action is logged for audit purposes.
                </p>
              </div>
            )}
          </div>
...
function SectionLabel({ children }) {
  return <p className="text-[10px] font-bold uppercase tracking-widest text-[#65a30d] mb-1">{children}</p>;
}
