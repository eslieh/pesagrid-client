...
export default function Sidebar({ currentPath, profile, subscription, onLogout, isOpen, onClose }) {
  return (
    <>
      {/* Backdrop for mobile */}
      <AnimatePresence>
      ...
      <aside 
        className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-zinc-100 bg-white transition-transform duration-300 ease-in-out lg:translate-x-0 ${
      ...
      >
        <div className="relative flex h-full flex-col py-6 px-4">
          ...
          <button 
            onClick={onClose}
            className="absolute top-6 right-4 p-2 text-zinc-400 hover:text-zinc-900 lg:hidden"
          >
